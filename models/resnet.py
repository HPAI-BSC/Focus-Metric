...
def conv1x1_modified(in_planes: int, out_planes: int, stride: int = 1) -> nn.Conv2d:
    """1x1 convolution"""
    return Conv2d(in_planes, out_planes, padding=1, kernel_size=3, stride=2, bias=False)
...
class ResNet(nn.Module):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            if stride == 2:
                downsample = Sequential(
                    conv1x1_modified(self.inplanes, planes * block.expansion, stride),
    ...
    def cascading_randomization(self, stop_layer) -> None:
        layers = [1, 8, 12, 17, 21, 27, 31, 39, 43, 49, 53, 61, 65, 71, 75, 83, 87, 91]
        len_layers = len(layers)
        for num, m in enumerate(self.modules()):
            if layers[len_layers - stop_layer] <= num:
