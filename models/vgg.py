...
class VGG(nn.Module):
    ...
    def cascading_randomization(self, stop_layer) -> None:
        cont_layers = 0
        total_layers = len([i for i in list(self.features) if isinstance(i, nn.Conv2d)]) + len(
            [i for i in list(self.classifier) if isinstance(i, nn.Linear)])
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                if cont_layers >= (total_layers - stop_layer):
                    print('m: ', m)
                    nn.init.kaiming_normal_(m.weight, mode='fan_out', nonlinearity='relu')
                    if m.bias is not None:
                        nn.init.constant_(m.bias, 0)
