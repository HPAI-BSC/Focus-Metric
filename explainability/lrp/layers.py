...
class BatchNorm2d(nn.BatchNorm2d, RelProp):
    def relprop(self, R, alpha):
        ...
        weight = self.weight.unsqueeze(0).unsqueeze(2).unsqueeze(3) / (
            (self.running_var.unsqueeze(0).unsqueeze(2).unsqueeze(3) + self.eps).pow(0.5))
        bias = self.bias.unsqueeze(0).unsqueeze(2).unsqueeze(3) - \
               ((self.running_mean.unsqueeze(0).unsqueeze(2).unsqueeze(3) * self.weight.unsqueeze(0).unsqueeze(
                   2).unsqueeze(3)) /
                (self.running_var.unsqueeze(0).unsqueeze(2).unsqueeze(3) + self.eps).pow(0.5))
        Z = X * weight + 1e-9
        Z = Z + (bias * torch.sign(Z) * torch.sign(bias))
        S = safe_divide(R, Z)
        R = X * self.gradprop(Z, X, S)[0]
